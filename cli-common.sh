...
# Detect whether any Java Agents should be used (only Open Telemetry for now!)
# Only enable if OpenTelemetry environment variables are present, excluding the OTEL_SERVICE_NAME variable that our
# calling scripts set!
env | grep "OTEL" | grep -v "OTEL_SERVICE_NAME" >/dev/null 2>&1
if [ $? -eq 0 ]; then
  for AGENT in "${SCRIPT_DIR}"/agents/*.jar; do
    [[ -e "$AGENT" ]] || break
